...
| 機能 | **広告ブロック** | DNS とは独立。ブラウザ内のリクエスト遮断で実現 |
| 機能 | **ブックマークのインポート** | 標準 HTML（Netscape）形式のファイルを取り込む |
| 機能 | **ブラウザに必要なセキュリティ** | WebView 堅牢化・権限管理・プライベートモード等 |

> 重要な整理：当初「カスタム DNS で広告ブロック」とされていたが、要件は **(a) セキュア DNS=DoH（プライバシー）** と **(b) 広告ブロック（ブラウザ内遮断）** の**2つの独立機能**に分離する。両者は別の仕組みで実現する。
...
| 広告ブロック | `WebViewClient.shouldInterceptRequest` ＋ フィルタリスト（ドメイン主体）＋ allowlist（§5.4） |
| ブックマークインポート | Netscape HTML を SAF ファイルピッカで取り込み（§5.5） |
...
**目的**：ブラウザ内で広告・トラッカのリクエストを遮断する。VPN や端末全体の変更は行わない（store にも優しい・軽量）。

**仕組み**
- `WebViewClient.shouldInterceptRequest(view, request)` で各リクエストの URL/ホストを検査。
  - 広告/トラッカに合致 → **空レスポンス（`WebResourceResponse`）を返して遮断**。
  - 非合致 → `null` を返して WebView に通常処理させる。
- **注意**：`shouldInterceptRequest` は POST のボディを取得できないため、**「遮断するか素通しするか」の判定専用**に使う（自前で全リクエストを取得し直す用途には使わない）。

**フィルタリスト（ドメイン主体でシンプルに）**
- ソース例（ON/OFF 可）：StevenBlack/hosts（統合）、AdGuard DNS filter、OISD、EasyList/EasyPrivacy のドメイン部。
- データ構造：**ドメインの HashSet ＋ サブドメイン照合**（左ラベルを 1 つずつ剥がして一致を探す）。O(1) 近傍で高速。
- 更新：定期＋手動。ローカルにキャッシュ。起動時はバックグラウンドでロードし起動を妨げない（§8）。
- **allowlist／サイト単位の無効化**：誤検知救済のため必須。
- （将来）URL パターン単位や EasyList の高度ルールは拡張（§13）。v1 はドメイン主体で十分。
...
- 主な項目：
  ...
  - **広告ブロック**：有効/無効、使用フィルタリスト、allowlist。
...
- **M2 — セキュア DNS（DoH）**
  - ローカルプロキシ実装（HTTP＋HTTPS CONNECT）。OkHttp DoH 解決。プロバイダ選択 UI。`ProxyController` 連携。非対応端末はプライベート DNS 案内へフォールバック。
- **M3 — 広告ブロック**
  - `shouldInterceptRequest` ＋ フィルタリスト（ドメイン主体）＋ allowlist／サイト単位無効化。リスト更新。
- **M4 — セキュリティ堅牢化**
...
**広告ブロック**
- 既知の広告/トラッカ ドメインがブラウザ内で遮断され、ページがそれらをロードしない。
- allowlist／サイト単位の無効化が効く（誤検知を救済できる）。
- VPN や端末全体設定を変更していない（ブラウザ内のみ）。
